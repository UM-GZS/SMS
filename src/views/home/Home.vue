<template>
	<el-container>
		...
		<el-container>
			<el-aside width="200px" v-show="showNav">
				...
				active-text-color="#409FFF" :default-active="activePath">
					<el-menu-item :index="item.path" v-for="(item, index) in menuList" :key="index" @click="navStatus(item.path)">
						<i :class="item.icon"></i>
...
<script>
	export default {
		...
		created() {
			var aPath = window.sessionStorage.getItem('activePath')
			if (aPath === null) return this.activePath = this.menuList[0].path
